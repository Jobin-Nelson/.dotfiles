...
declare user_hyprland_conf="$HOME/.config/hypr/hyprland.conf"
declare -a hyprland_bindings_conf=(
  "$HOME/.config/hypr/modules/bindings/tiling.conf"
  "$HOME/.config/hypr/modules/bindings/utilities.conf"
  "$HOME/.config/hypr/modules/bindings/media.conf"
)
...
grep -h '^[[:space:]]*bind' "${user_hyprland_conf}" "${hyprland_bindings_conf[@]}" |
  sed 's/#.*//' |
  sed '/^[[:space:]]*$/d' |
  sort -u |
  awk -F, '

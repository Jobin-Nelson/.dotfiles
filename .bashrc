...
alias rwl='if [[ $XDG_CURRENT_DESKTOP = "GNOME" ]]; then gsettings set org.gnome.desktop.background picture-uri-dark "file://$(find $HOME/Pictures/wallpapers -type f | shuf -n 1)"; elif [[ $XDG_CURRENT_DESKTOP == "Hyprland" ]]; then reload.sh -p $(find $HOME/Pictures/wallpapers -type f -name '"'"'*.png'"'"' -or -name '"'"'*.jpg'"'"' | shuf -n 1); fi'
